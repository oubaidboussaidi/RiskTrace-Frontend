...
export class AvatarComponent implements OnInit, OnDestroy, OnChanges {
  /** Fallback name for initials and background color */
  @Input() name: string = '';

  /** Target image URL or Base64 data */
  ...
  /** Owner ID for cache and notifications */
  @Input() entityId: string = '';

  /** Owner entity type */
  @Input() entityType: 'user' | 'org' = 'user';

  /** Event emitted on avatar upload/remove */
  ...
  private refresh() {
    ...
    // Resolve image: Input > Cache > None
    if (this.imageUrl) {
